...
- 주문 입력과 포트폴리오 가시성을 제공하는 대시보드 + 선택 심볼 상세 워크스페이스

## 사용자 관점의 현재 제품 표면

외부 사용자나 저장소 방문자 기준에서 현재 제품 경험은 다음과 같습니다.
- 웹 프론트엔드의 대시보드 페이지 `/`
- 선택 종목 전용 상세 페이지 `/instruments/[symbol]`
- 미국 주식 7개 심볼로 구성된 내장 데모 카탈로그 검색
- 검색 결과의 `Open detail`, 포지션 심볼, 최근 주문 심볼에서 같은 상세 페이지로 이동 가능
- 선택한 종목의 시세 패널
- 결정적 `1D` 캔들 데이터를 사용하는 일봉 차트 패널
- 선택 종목 심볼을 미리 채워 넣는 페이퍼 주문 폼
- 상세 페이지에서는 하나의 심볼이 시세, 차트, quant signal, 주문 패널 전체의 공통 상태로 유지됨
