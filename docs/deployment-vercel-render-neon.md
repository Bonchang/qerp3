...
- 프론트엔드는 `NEXT_PUBLIC_API_BASE_URL` 로 백엔드 주소를 바꿀 수 있고, Vercel 배포에서 값이 비어 있으면 `https://qerp3-backend.onrender.com` 으로 자동 폴백합니다.
...
기본적으로 프론트엔드는 Vercel 서버에서 `NEXT_PUBLIC_API_BASE_URL` 값을 사용합니다. 값을 비워 두면 현재 라이브 Render 백엔드인 `https://qerp3-backend.onrender.com` 으로 요청하고, 브라우저는 같은 프론트엔드 도메인의 `/api/backend/*` 프록시만 호출합니다.
...
| 변수 | 필수 | 비고 |
| --- | --- | --- |
| `NEXT_PUBLIC_API_BASE_URL` | 아니오 | 지정하면 해당 백엔드 URL 사용, 비우면 Vercel에서 `https://qerp3-backend.onrender.com` 자동 사용 |
