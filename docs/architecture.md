...
| 구성 요소 | 책임 |
| --- | --- |
| 프론트엔드 | 종목 검색, 선택 심볼 상세 딥링크, 시세/차트 표시, 주문 제출, 포트폴리오 및 최근 주문 렌더링 |
...
| 진입점 | 대상 | 현재 계약 |
| --- | --- | --- |
| `/` | 최종 사용자 | 종목 검색 허브와 포트폴리오 요약, 포지션, 최근 주문을 제공하는 메인 대시보드 |
| `/instruments/[symbol]` | 최종 사용자 | 선택 심볼의 전용 상세 화면. 같은 심볼이 시세, 차트, quant signal, 주문 입력 패널 전체를 함께 구동합니다. |
| `/api/backend/*` | 프론트엔드 런타임 | 브라우저 요청을 백엔드 API로 전달하는 Next.js 프록시 경로 |
| `/api/v1/instruments/*` | 프론트엔드 프록시 또는 외부 API 소비자 | 내장 데모 종목 카탈로그 검색과 심볼 메타데이터 조회 |
...
대시보드는 심볼 탐색의 시작점이고, 검색 결과의 `Open detail` 액션과 포지션/최근 주문 심볼 링크는 모두 `/instruments/[symbol]` 딥링크로 수렴합니다.
